...
if (!empty($liste_cat) && !empty($id_cat)) {
    foreach ($liste_cat as $val_select) {
        ?>
							<option value="<?php echo $val_select->fte_categories_id; ?>" <?php if ($id_cat == $val_select->fte_categories_id) {
            echo "selected";
        }
        ?>><?php echo ascii_to_entities($val_select->libelle_categories); ?></option>
							<?php
}
}
?>
...
echo "var collapse_id = \"#collapse-" . $id_collapse . "-1\";";
                ...
                ?>
									...
									if(obj.icon == "jstree-default jstree-file"){
										return true;
									/*}else if((obj.children.length == obj.children_d.length) && (obj.children.length != 0 && obj.children_d.length != 0)){
										return true;
									...
									*/
									...
									"_disabled"			: function (data) {
									var inst = $.jstree.reference(data.reference),
									obj = inst.get_prev_dom(data.reference);

										//console.log(obj);

									if(obj.icon == "jstree-default jstree-folder" && obj2.icon != "jstree-default jstree-folder"){
										return true;
									}else{
										return false;
									}
