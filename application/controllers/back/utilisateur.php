...
class Utilisateur extends CI_Controller
{
	...
	public function utilisateur()
	{
		...
		if($this->session->userdata('loggin') && $level == 'admin'){
			...
			$data['prenom_admin']= $this->session->userdata('prenom');
	...
	public function ajouter(){
		...
		if($this->session->userdata('loggin') && $level == 'admin' && $this->input->post('ajax') == '1'){
			...
			if($this->form_validation->run()) {
				...
				$result = $this->email->from("tolotra_si@vivetic.mg", "Outil d'aide à l'agent ADLP")
					->to($mail)
					->subject("Création de votre compte utilisateur Outil d'aide à l'agent ADLP")
					->message($view_mail)
					->send();
		...
		if($this->input->post('ajax') == '1'){
			...
			if($this->input->post('modif') == 'mail_renseigne'){
				...
				if($this->form_validation->run()) {
					...
					$result = $this->email->from("tolotra_si@vivetic.mg", "Outil d'aide à l'agent ADLP")
										->to($mail)
										->subject("Renseignement de votre adresse E-mail dans l'Outil d'aide à l'agent ADLP")
										->message($view_mail)
										->send();
				...
				if($this->form_validation->run()) {
					...
					$result = $this->email->from("tolotra_si@vivetic.mg", "Outil d'aide à l'agent ADLP")
						->to($mail)
						->subject("Modification mot de passe et renseignement de votre adresse E-mail dans l'Outil d'aide à l'agent ADLP")
						->message($view_mail)
						->send();
